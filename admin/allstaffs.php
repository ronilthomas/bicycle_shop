  ...
  if(isset($_POST['option']))
{
  ...
  $course = $mysqli->escape_string($_POST['option']);
  if($course == 'View Staff')
  {
    $option='stafftable';
  }
  else
...
<div class="container-fluid">
  <div style="overflow-y: scroll; height: 150px; width: 100%">  <!-- to seperate table from form -->
...
while ($ppl = $result->fetch_assoc()) {?>
  ...
  <td>  <?php echo $ppl['email'];  ?></td>
  <td id="<?php echo $ppl['id'].'actualvalue'; ?>">  <?php echo $ppl['active'];  ?>  </td>
...
<form method="POST">
  ...
  <button id="formsubmitbtn" type="submit" class="btn btn-primary" name="adminaddstudent">Add Staff</button>
</form>
</div>
...
if(isset($_POST['adminaddstudent']))
{
  ...
  if($_POST['useremail'] && $_POST['userfirstname'] != '')
  {
    ...
    $sql = "SELECT * FROM $option WHERE email = ?";
      ...
      {
        if(!$stmt = $mysqli->prepare("INSERT INTO $option (registered_date, first_name, email, password, active) VALUES (?, ?, ?, ?, ?)"))
        {
          echo "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error;
        }

        if(!$stmt->bind_param('dsssi', $date, $userfirstname, $useremail, $passwordtodb, $active))
        {
          echo "Binding failed: (" . $mysqli->errno . ") " . $mysqli->error;
        }
        //$stmt1->execute();
...
if($_POST['useremail'] && $_POST['userfirstname'] != '')
  {
    ...
    if(!$stmt=$mysqli->prepare($sql))
      ...
      {
        $sql = "UPDATE $option SET first_name=?, email=? WHERE id=?";
        if(!$stmt=$mysqli->prepare($sql))
        {
          echo "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error;
        }
...
if($_POST['useremail'] && $_POST['userfirstname'] != '')
  {
    ...
    $sql = "DELETE FROM $option WHERE email=?";
    if(!$stmt=$mysqli->prepare($sql))
      {
        echo "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error;
      }
